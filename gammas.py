...
def compute_ratio(m, w, A=129, Z=54):
    mnu = mp.mpf(m) 
    w0 = mp.mpf(w)
...
# # Mass generation function (NH/IH/degenerate)
# def get_masses(m_sum=None, m0=None, hierarchy="normal", mode="exact"):
#     dm21 = mp.mpf("7.4e-5")
#     dm31 = mp.mpf("2.5e-3")
...
#             def f(m1): return m1 + sqrt(m1**2 + dm21) + sqrt(m1**2 + dm31) - m_sum
#             m1 = findroot(f, 0.001)
#         else:
#             m1 = mp.mpf(m0)
#         return [m1, sqrt(m1**2 + dm21), sqrt(m1**2 + dm31)]

#     elif hierarchy == "inverted":
#         if m_sum is not None:
#             def f(m3): return sqrt(m3**2 + dm31) + sqrt(m3**2 + dm31 + dm21) + m3 - m_sum
#             m3 = findroot(f, 0.001)
#         else:
#             m3 = mp.mpf(m0)
#         m1 = sqrt(m3**2 + dm31)
#         m2 = sqrt(m3**2 + dm31 + dm21)
#         return [m1, m2, m3]

#     else:
#         raise ValueError("Hierarchy must be 'normal', 'inverted', or use mode='degenerate'.")

# def compute_ratio(w, m_sum=None, m0=None, hierarchy="normal", mode="exact"):
#     w0 = mp.mpf(w)
#     mm = get_masses(m_sum=m_sum, m0=m0, hierarchy=hierarchy, mode=mode)
#     gp_eV, gm_eV, gp_Hz, gm_Hz = compute_gammas(w0, mm, gav)
#     return gp_eV / gm_eV, gm_eV
...
# for i in range(3):
#     for j in range(3):
#         gav_Xe[i, j] = sum(ga_Xe[k] * np.conj(U[k, i]) * U[k, j] for k in range(3))
